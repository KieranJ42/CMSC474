...
public class PublicTests {
  ...
  @Test public void testPublic1() {
    assertEquals(0, new BusTerminal().numBusesAtTerminal());
  }
  ...
  // numPeopleOnBus(int) and numPeopleOnBus(int, String).
  ...
  // Tests calling numPeopleOnBus() on a bus that has more than one
  // passenger with the same name.
  @Test public void testPublic9() {
    BusTerminal busTerminal= TestData.busTerminal5();

    assertEquals(3, busTerminal.numPeopleOnBus(132, "Sheep"));
  }

  // Tests calling numBusesGoingToCity() on a busTerminal with multiple buses.
  ...
  // Tests trying to add more than 25 buses to a busTerminal, which should
  // not succeed.
  @Test public void testPublic14() {
    BusTerminal busTerminal= TestData.busTerminal7();
    int i;

    // should not succeed
    busTerminal.addBusToTerminal(999, 100, Arrays.asList("Novosibirsk"));

    // make sure the addBusToTerminal() call above did not have any effect
    assertEquals(25, busTerminal.numBusesAtTerminal());
    for (i= 1; i <= 25; i++)
      assertEquals(1, busTerminal.numBusesGoingToCity("City" + i));
    assertEquals(0, busTerminal.numBusesGoingToCity("Novosibirsk"));
  }
