...
public class SMkdTreeMe<LPoint extends LabeledPoint2D> {
	...
	private abstract class Node {
		...
		abstract ArrayList<String> listNodesWithCenters(Rectangle2D cell);
	...
	private class InternalNode extends Node {
		...
		public InternalNode(int cutDim, double cutVal, Rectangle2D cell, LinkedList<LPoint> contends) {
			...
			this.cell = cell;
			contenders = fixContenders(contenders);
			this.left = new ExternalNode(null, contenders);
			this.right = new ExternalNode(null, contenders);
		}
		...
		public LinkedList<LPoint> fixContenders(LinkedList<LPoint> contenders) {
			...
			double tempMin;
			min = max4(temp.getPoint2D().distanceSq(cell.high),
			...
			for (int i = 1; i < contenders.size(); i++) {
				...
				tempMin = max4(temp.getPoint2D().distanceSq(cell.high),
						temp.getPoint2D().distanceSq(cell.high),
						temp.getPoint2D().distanceSq(
								new Point2D(cell.high.getX(),cell.low.getY())),
						temp.getPoint2D().distanceSq(
		...
		Node bulkCreate(ArrayList<LPoint> points, Rectangle2D cell) {
			...
			if (points.size()==0) {
				return new ExternalNode(null,contenders);
			}
			if (points.size()==1) {
				return new ExternalNode(points.get(0),contenders);
			}
		...
		ArrayList<String> listNodesWithCenters(Rectangle2D cell) {
			ArrayList<String> retVal = new ArrayList<String>();
			String temp;
			if (cutDim==0)
			...
			retVal.addAll(right.listNodesWithCenters(lcell));
			retVal.addAll(left.listNodesWithCenters(rcell));
			return retVal;
		}
	}
	...
	private class ExternalNode extends Node {
		...
		ExternalNode(LPoint point, LinkedList<LPoint> contends) {
			this.point = point;
			contenders = new LinkedList<LPoint>(contends);
		}
		
		ExternalNode(LPoint point, LPoint startCenter) {
	...
	public SMkdTreeMe(int rebuildOffset, Rectangle2D rootCell, LPoint startCenter) { 
		REBUILDOFFSET = rebuildOffset;
		this.rootCell = rootCell;
		root = new ExternalNode(null,startCenter);
	...
	ArrayList<String> listWithCenters() { return root.listNodesWithCenters(rootCell); }
	
	ArrayList<ClusterAssignment<LPoint>.AssignedPair> listAssignments(ClusterAssignment<LPoint> ca) {
		ArrayList<ClusterAssignment<LPoint>.AssignedPair> assignments = new ArrayList<ClusterAssignment<LPoint>.AssignedPair>();
		root.makeAssignments(ca);
		return assignments;
	}
}
